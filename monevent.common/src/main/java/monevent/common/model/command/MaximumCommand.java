 ...
 */
public class MaximumCommand extends Command {
    public static String Type = "MAXIMUM";
    ...
    public MaximumCommand(String sourceFieldToCompare, String destinationFieldToCompare, String destinationField) {
        super(MaximumCommand.Type);
    ...
    public static Command Parse(List<String> tokens) {
        ...
        if (tokens.size() == 4) {
            return new MaximumCommand(tokens.get(1), tokens.get(2), tokens.get(3));
        } else {
            throw new CommandException("Invalid command line for maximum command. Must be : MAXIMUM sourceFieldToCompare destinationFieldToCompare destinationField");
        }
    }
}
